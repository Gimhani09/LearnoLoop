...
const PostDetail = ({ user }) => {
    ...
    useEffect(() => {
        if (post && user) {
            // Get all possible ID values from the post
            const postOwnerId = post.userId || post.user_id || post.authorId || post._id;
            const userId = user.id || user._id;
            
            // Get usernames for comparison
            const postUsername = post.username || post.author || '';
            const username = user.username || '';
            
            console.log('Checking ownership - userId:', userId, 'postOwnerId:', postOwnerId, 'post username:', postUsername, 'user username:', username);
            
            // Check ownership using both ID and username
            const isOwnerById = postOwnerId && userId && (postOwnerId === userId);
            const isOwnerByUsername = postUsername && username && (postUsername === username);
            
            setIsOwner(isOwnerById || isOwnerByUsername || (user.role === 'ADMIN'));
            fetchLikes();
            checkIfSaved();
        }
    }, [post, user]);
